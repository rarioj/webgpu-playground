...
fn intersectRaySphere(ray: Ray, sphere: Sphere, tMin: f32, tMax: f32, oldState: RenderState) -> RenderState {
    ...
    if discriminant > 0.0 {
        ...
        if t > tMin && t < tMax {
            ...
            newState.normal = normalize(newState.position - sphere.center);
            newState.t = t;
    ...
    while true {
        ...
        if sphereCount == 0u {
            ...
            for (var i: u32 = 0u; i < sphereCount; i++) {
                var newState: RenderState = intersectRaySphere(ray, objects.spheres[sphereLookup.sphereIndices[i + leftChild]], 0.001, nearestHit, renderState);
                if newState.hit {
