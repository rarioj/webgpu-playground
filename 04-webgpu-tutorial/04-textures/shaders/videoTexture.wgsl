...
@group(1) @binding(0) var TextureView: texture_external;
...
fn fragmentMain(@location(0) fragmentUV: vec2<f32>, @location(1) fragmentPosition: vec4<f32>) -> @location(0) vec4<f32> {
    // return textureSampleBaseClampToEdge(TextureView, Sampler, fragmentUV) * fragmentPosition;
    return textureSampleBaseClampToEdge(TextureView, Sampler, fragmentUV);
}